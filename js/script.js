     ...
     switch (url.url){
            ...
            case "matchId":
               async function matchId(id) {
                   ...
                   const notificationSaved = document.getElementById('notificationSaved');
                   getById("matchId-"+url.id).then(function (data) {
                       if (data !== undefined) {
                            //kode untuk menghilangkan button save jika data sudah disimpan sebelumnya
                            save.classList.add("hide");
                            //kode untuk menampilkan pesan bahwa halaman telah tersimpan
                            notificationSaved.classList.remove("hide");
                       }
                    });
                   ...
                   save.addEventListener('click', async function() {
                        ...
                        //kode untuk menghilangkan button jika button di klik
                        save.classList.add("hide");
                        //kode untuk menampilkan pesan bahwa halaman telah tersimpan
                        notificationSaved.classList.remove("hide");
                    });
            ...
            case "standing":
                 async function standing() {
                    ...
                    const save = document.getElementById('save');
                    const notificationSaved = document.getElementById('notificationSaved');
                    getById("standings").then(function (data) {
                        if (data !== undefined) {
                            //kode untuk menghilangkan button save jika data sudah disimpan sebelumnya
                            save.classList.add("hide");
                            //kode untuk menampilkan pesan bahwa halaman telah tersimpan
